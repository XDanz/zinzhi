 ...
 *  from the  segragetad list.
 * It will determine which linked list  to search from by calling
 * segr_index().
 *
 */
static void segr_remove(void *ptr){
  char ** heap_ptr;
  char ** addr;
  char ** prev;
 ...
 * If split occures then put the remaing free block in the
 * segregated list.
 */
static void place(void *bp, size_t asize){
