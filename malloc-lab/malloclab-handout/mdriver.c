...
int libc_thruput;	/* Thruput for libc on TEST_FILE */
...
int main(int argc, char **argv)
{
    ...
    /*
     * Calculate libc_thruput
     */
    {
	trace = read_trace(TRACEDIR, TEST_FILE);
	speed_params.trace = trace;
	libc_thruput = trace->num_ops / fsecs(eval_libc_speed, &speed_params);
	if (verbose) {
            printf ("Libc throughput = %.0f Kops\n", libc_thruput/1E3);
	}
	free_trace(trace);
    }
    ...
    printf("%d correct traces and %d errors gives %d correctness points\n",
	   numcorrect,
	   errors,
	   numcorrect*TRACE_CORRECT_POINTS);

    if (numcorrect != 0) {
	avg_mm_throughput = ops/secs;
	avg_mm_util = util/numcorrect;

	p1 = UTIL_WEIGHT * avg_mm_util;
//	if (avg_mm_throughput > LIBC_THRUPUT) {
	if (avg_mm_throughput > libc_thruput) {
	    p2 = (double)(1.0 - UTIL_WEIGHT);
	} 
	else {
	    p2 = ((double) (1.0 - UTIL_WEIGHT)) * 
		(avg_mm_throughput/libc_thruput);
//		(avg_mm_throughput/AVG_LIBC_THRUPUT);
	}
	
	perfindex = (p1 + p2)*100.0;
	printf(
 "Perf index = %.0f (util) + %.0f (thru) = %.0f/100 gives %.0f performance points\n",
	       p1*100, 
	       p2*100, 
	       perfindex,
	       perfindex*PERF_POINTS/100.0);
