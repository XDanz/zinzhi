...
public class test {
    ...
    static public void main(String args[]) {
        ...
        System.err.println("---------------------------------------");
        System.err.println("Java CDB tests");
        System.err.println("---------------------------------------");
        ...
        TestCase[] tests = new TestCase[] {
            ...
            new TestCase() {
                public void test() throws Exception {
                    ...
                    if (n != num_instances2) {
                        failed("expected " + num_instances2 +
                               " instances but got " + n);
                    }
                    ...
                    for (int i=0; i<n; i++) {
                        ...
                        for (int k=0; k<objArr.length; k++) {
                            ...
                            if (k==0) {
                                ...
                                if (!"12345678901234567890".equals(obj
                                                                   .toString())) {
                                    failed("expected elemvalue 12345678901234567890 but got " + obj);
                                }
                            }
            ...
            new TestCase() {
                public void test() throws Exception {
                    ...
                    if (n != num_instances1) {
                        failed("expected " +
                               num_instances1 + " instances but got " + n);
                    }
                    ...
                    for (ConfObject[] objArr : objList) {
                        ...
                        if (num_elems1 != objArr.length) {
                            failed("for instance "+i+" expected " +
                                   num_elems1 +
                                   " elements but got " + objArr.length);
                        }
                    ...
                    objList =
                        sess.getObjects(num_elems2, 0, num_instances2, "entry");
                    ...
                    for (ConfObject[] objArr : objList) {
                        ...
                        for (int k=0; k<objArr.length; k++) {
                            ...
                            if (k==0) {
                                if (!obj
                                    .toString().equals(String.valueOf(i+1))) {
                                    failed("expected elemvalue " + (i+1)
                                           + " but got " + obj);
                                }
                            } else {
                                if (!"12345678901234567890"
                ...
                public void test() throws Exception {
                    ...
                    params = new ConfXMLParam[] {
                        ...
                        new ConfXMLParamLeaf(b.hash(),
                                             ...
                                             cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(),
                                             b.toHash(leaf_names[14]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[15]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[16]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[17]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[18]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[19]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[20]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[21]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[22]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[23]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[24]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[25]), cdb.getNsList()),
                        new ConfXMLParamLeaf(b.hash(), b.toHash(leaf_names[26]), cdb.getNsList()),
                        new ConfXMLParamStop(b.hash(), b.toHash("entry"), cdb.getNsList())
                    };
                    ...
                    if (resTag.length != num_elems1+2) {
                        failed ("expected " + num_elems1 + " elements but got " + resTag.length);
                    }
                    ...
                    for (ConfXMLParam param : resTag) {
                        if (offset==0) {
                                ...
                                if (!param.val.toString().equals(String.valueOf(1))) {
                                        failed("expected elemvalue 1 but got " + param.val);
                                }
                        } else if (offset <= num_elems1) {
                                if (!"12345678901234567890".equals(param.val.toString())) {
                                        failed("expected elemvalue 12345678901234567890 but got " + param.val);
                                }
                        }
                        offset++;
                    }
