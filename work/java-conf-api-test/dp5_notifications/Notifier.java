...
public class Notifier implements DpNotifReplayCallback {
    ...
    class Replay {
        ConfDatetime time;
        ConfXMLParam[] event;
        Replay(ConfDatetime time, ConfXMLParam[] event) {
            this.event = event;
            this.time = time;
        }
    }
     ...
     */
    public void send_notification(ConfXMLParam[] params)
        throws ConfException, IOException {
        ConfDatetime now = new ConfDatetime();
        replayBuffer.add( new Replay(now, params) );
        stream.send( now, params );
    }

    /**
     *  flush a notifications in ConfD
     */
    public void flush()
        throws ConfException, IOException {
        stream.flush();
