...
#define ncs_add cs_add
#define ncs_sub cs_sub
#define ncs_mul cs_mul
#define ncs_div cs_div
#define ncs_square cs_square
...
#define EXPECT_PARAMS {                                 \
        TAG(cs_operation, BEGIN(cs_operation)),         \
        TAG(cs_number, INT32(13)),                      \
        TAG(cs_type, ENUM(cs_add)),                     \
        TAG(cs_operands, LIST(expect_list_add)),        \
        TAG(cs_operation, END(cs_operation)),           \
        TAG(cs_operation, BEGIN(cs_operation)),         \
        TAG(cs_number, INT32(1)),                       \
        TAG(cs_type, ENUM(cs_square)),                  \
        TAG(cs_operands, LIST(expect_list_square)),     \
        TAG(cs_operation, END(cs_operation)),           \
        TAG(cs_operation, BEGIN(cs_operation)),         \
        TAG(cs_number, INT32(42)),                      \
        TAG(cs_type, ENUM(cs_div)),                     \
        TAG(cs_operands, LIST(expect_list_div)),        \
        TAG(cs_operation, END(cs_operation))            \
    }
...
#define NS cs__ns
static struct xml_tag c_expect_name = { .tag = cs_math, .ns = NS };
...
static confd_tag_value_t c_expect_values[] = {
        TAG(cs_result, BEGIN(cs_result)),
        TAG(cs_number, INT32(13)),
        TAG(cs_type, ENUM(cs_add)),
        TAG(cs_value, INT16(38)),
        TAG(cs_result, END(cs_result)),
        TAG(cs_result, BEGIN(cs_result)),
        TAG(cs_number, INT32(1)),
        TAG(cs_type, ENUM(cs_square)),
        TAG(cs_value, INT16(49)),
        TAG(cs_result, END(cs_result)),
        TAG(cs_result, BEGIN(cs_result)),
        TAG(cs_number, INT32(42)),
        TAG(cs_type, ENUM(cs_div)),
        TAG(cs_value, INT16(6)),
        TAG(cs_result, END(cs_result))
};

static confd_tag_value_t optparam[] = {
        TAG(cs_maybe, INT32(13))
};
                ...
                int nparams, int ns)
{
    ...
    if ((i = CONFD_GET_INT32(CONFD_GET_TAG_VALUE(&params[1]))) <= 0) {
        ...
        case -5: {
            ...
            j = 0;
            CONFD_SET_TAG_XMLBEGIN(&reply[j],cs_result, ns); j++;
            CONFD_SET_TAG_INT32(&reply[j], cs_number, i); j++;
            type = CONFD_GET_ENUM_HASH(CONFD_GET_TAG_VALUE(&params[2]));
            CONFD_SET_TAG_ENUM_HASH(&reply[j], cs_type, type); j++;
            CONFD_SET_TAG_INT16(&reply[j], cs_value, 14); j++;
            CONFD_SET_TAG_XMLEND(&reply[j],cs_result, ns); j++;
    ...
    for (i = 0; i < expect_nparams; i++) {
        ...
        if (params[i].tag.tag == cs_operation &&
            params[i].v.type == C_XMLBEGIN)
            num_ops++;
    }

    {
        ...
        for (op = 0; op < num_ops; op++) {
            CONFD_SET_TAG_XMLBEGIN(&reply[j],cs_result, ns); j++;
            i++;                /* skip operation begin */
            CONFD_SET_TAG_INT32(&reply[j], cs_number,
                                CONFD_GET_INT32(
                                   CONFD_GET_TAG_VALUE(&params[i]))); i++; j++;
            type = CONFD_GET_ENUM_HASH(CONFD_GET_TAG_VALUE(&params[i])); i++;
            CONFD_SET_TAG_ENUM_HASH(&reply[j], cs_type, type); j++;
            oplist = CONFD_GET_LIST(CONFD_GET_TAG_VALUE(&params[i])); i++;
            ...
            CONFD_SET_TAG_INT16(&reply[j], cs_value, result); j++;
            CONFD_SET_TAG_XMLEND(&reply[j], cs_result, ns); j++;
        }
        confd_action_reply_values(uinfo, reply, j);
    }
                   ...
                   int nparams)
{
    ...
    fprintf(stderr, "** ---> math_cs action called\n");
    assert(kp != NULL);
...
static void request_action(struct addrinfo *addr)
{
    ...
    assert(maapi_set_namespace(sock, th, cs__ns) == CONFD_OK);
    ...
    /* wrong tag in params */
    CONFD_SET_TAG_INT32(&params[11], cs_value, 42);
    ERRTEST(CONFD_ERR_BADTYPE, "Missing value");
    CONFD_SET_TAG_INT32(&params[11], cs_number, 42);

    /* wrong type in params */
    CONFD_SET_TAG_INT32(&params[7], cs_type, cs_square);
    ERRTEST(CONFD_ERR_BADTYPE, "expected type");
    CONFD_SET_TAG_ENUM_HASH(&params[7], cs_type, cs_square);
    ...
    CONFD_SET_TAG_INT32(&params[expect_nparams], cs_value, 42);
    ...
    CONFD_SET_TAG_XMLBEGIN(&params[5], cs_operation, cs__ns);
    ...
    CONFD_SET_TAG_INT32(&params[1], cs_number, 0);
    assert(maapi_request_action(sock, params, nparams,
                                &values, &nvalues, cs__ns,
    ...
    /* ERR return w/o msg */
    CONFD_SET_TAG_INT32(&params[1], cs_number, -1);
    ERRTEST(CONFD_ERR_EXTERNAL, "");

    /* bogus err return */
    CONFD_SET_TAG_INT32(&params[1], cs_number, -2);
    ERRTEST(CONFD_ERR_EXTERNAL, "Bad return value: -17");

    /* ERR return w msg */
    CONFD_SET_TAG_INT32(&params[1], cs_number, -3);
    ERRTEST(CONFD_ERR_EXTERNAL, "Number -3 is invalid");

    /* OK return, no values, msg thrown away */
    CONFD_SET_TAG_INT32(&params[1], cs_number, -4);
    assert(maapi_request_action(sock, params, nparams,
                                &values, &nvalues, cs__ns,
                                "/system/computer{fred}/math") == CONFD_OK);
    assert(nvalues == 0);
    ...
    CONFD_SET_TAG_INT32(&params[1], cs_number, -5);
    assert(maapi_request_action(sock, params, nparams,
                                &values, &nvalues, cs__ns,
                                "/system/computer{fred}/math") == CONFD_OK);
    assert(nvalues == 5);
    ...
    if (nvalues > 0)
        free(values);
    nvalues = 0;
    CONFD_SET_TAG_INT32(&params[1], cs_number, 13);
    ...
    CONFD_SET_TAG_INT32(&params[1], cs_number, 47);
    ERRTEST(CONFD_ERR_EXTERNAL, "");
...
int main(int argc, char **argv)
{
    ...
    strcpy(acb.actionpoint, "math_cs");
