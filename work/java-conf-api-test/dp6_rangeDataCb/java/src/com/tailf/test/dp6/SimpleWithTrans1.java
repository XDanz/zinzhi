...
public class SimpleWithTrans1 {

    private static Logger LOGGER = Logger.getLogger(SimpleWithTrans1.class);
    /**
    ...
    public synchronized static void save(String filename) throws IOException {
        try {
            ...
            LOGGER.error("failed to save: "+filename+ " got: "+e.getMessage());
            throw e;
        }
    }
    ...
    static public void main(String args[]) {
        ...
        try {
            ...
            SimpleTransCb1 trans_cb = new SimpleTransCb1();
            SimpleDataCb1 data_cb1 = new SimpleDataCb1();
            SimpleDataCb2 data_cb2 = new SimpleDataCb2();

            /* create new control socket */
            Socket ctrl_socket= new Socket("127.0.0.1",Conf.PORT);

            /* init and connect the control socket */
            dp = new Dp("server_daemon1",ctrl_socket);
            // Dp dp = new Dp("server_daemon1",ctrl_socket);

            /* register the callbacks */
            dp.registerAnnotatedCallbacks( trans_cb );
            dp.registerAnnotatedRangeDataCallbacks(
                                         data_cb1,
                                         new ConfValue[] { new ConfBuf("a") },
                                         new ConfValue[] { new ConfBuf("cz") },
                                         new ConfPath("/servers/server"));
            dp.registerAnnotatedRangeDataCallbacks( data_cb2,
                                          new ConfValue[] { new ConfBuf("d") },
                                          new ConfValue[] { new ConfBuf("fz") },
                                          new ConfPath("/servers/server"));
            dp.registerDone();
